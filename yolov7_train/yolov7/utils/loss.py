...
class RankSort(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta_RS=0.50, eps=1e-10): 
        ...
        classification_grads=torch.zeros(logits.shape).cpu()
        ...
        relevant_bg_grad=torch.zeros(len(relevant_bg_logits)).cpu()
        sorting_error=torch.zeros(fg_num).cpu()
        ranking_error=torch.zeros(fg_num).cpu()
        fg_grad=torch.zeros(fg_num).cpu()
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5): 
        ...
        relevant_bg_logits=logits[relevant_bg_labels] 
        relevant_bg_grad=torch.zeros(len(relevant_bg_logits)).cpu()
        rank=torch.zeros(fg_num).cpu()
        prec=torch.zeros(fg_num).cpu()
        fg_grad=torch.zeros(fg_num).cpu()
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, models
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                from_which_layer.append((torch.ones(size=(len(b),)) * i).to('cpu'))
